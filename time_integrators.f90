...
do ! while (time < t_final)
   ...
   start = OMP_GET_WTIME()
   !$OMP PARALLEL DO num_threads(1) private(tmp_phi, tmp_T, tmp_uy, tmp_phi1, tmp_uy1, tmp_K_phi, tmp_K_T) schedule(dynamic)
   do it = 1,Nx ! kx loop
      ! Compute phi1 and T1
   ...
   ! Get ux and uy
   !$OMP PARALLEL DO num_threads(1) private(tmp_uy, it) schedule(dynamic)
   do it = 1,Nx
      ! Solve for v
...
select case(stage)
   case (1)
      !$OMP PARALLEL DO num_threads(1) schedule(dynamic)
      do i = 1,Nx
...
start = OMP_GET_WTIME()
!$OMP PARALLEL DO num_threads(1) private(tnlT, tnlphi, tT, tux, tuy, tphi) schedule(dynamic)
do j = 1,Ny
...
start = OMP_GET_WTIME()
!$OMP PARALLEL DO num_threads(1) private(tmp_T) schedule(dynamic)
do i = 1,Nx
   ! Temperature
   tmp_T = Ti(:,i)
...
start = OMP_GET_WTIME()
!$OMP PARALLEL DO num_threads(1) private(tnlT, tnlphi) schedule(dynamic)
do j = 1,Ny
   tnlT   = nlT(j,:)
   tnlphi = nlphi(j,:)
