module time_integrators
   ...
   complex(C_DOUBLE_COMPLEX),              dimension(1275)  :: phiin1
   complex(C_DOUBLE_COMPLEX), allocatable, dimension(:)    :: vi1
   real(dp),                  allocatable, dimension(:,:)               :: vi_rhs1
   real(dp),                  allocatable, dimension(:)                 :: dvi1, dlvi1, duvi1
   integer                                                              :: j1
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   
   allocate(vi1(Ny), stat=alloc_err)
   call check_alloc_err(alloc_err)
   allocate(vi_rhs1(Ny-2,2), stat=alloc_err)
   call check_alloc_err(alloc_err)
   allocate(dvi1(Ny-2), stat=alloc_err)
   call check_alloc_err(alloc_err)
   allocate(dlvi1(Ny-3), duvi1(Ny-3), stat=alloc_err)
   call check_alloc_err(alloc_err)
   ...
   !   !$OMP END PARALLEL DO
      ...
      ! Get ux and uy
      !$OMP PARALLEL DO num_threads(8) private(tmp_uy, it, phiin1, vi1, vi_rhs1, dvi1, dlvi1, duvi1, j1) schedule(dynamic)
      do it = 1,Nx
         ! Solve for v
         phiin1 = phi(:,it) 
         vi1     = cmplx(0.0_dp, 0.0_dp, kind=C_DOUBLE_COMPLEX)
         vi_rhs1 = 0.0_dp
         dvi1    = 0.0_dp
         dlvi1   = 0.0_dp
         duvi1   = 0.0_dp
         ...
         vi_rhs1(:,1) = real (phiin1(2:Ny-1))
         vi_rhs1(:,2) = aimag(phiin1(2:Ny-1))
         
         call dgtsv(Ny-2, 2, dlvi1, dvi1, duvi1, vi_rhs1, Ny-2, info)
         
         vi1(1)   = cmplx(0.0_dp, 0.0_dp, kind=C_DOUBLE_COMPLEX)
         vi1(2:Ny-1) = cmplx(vi_rhs1(:,1), vi_rhs1(:,2), kind=C_DOUBLE_COMPLEX)
         vi1(Ny)  = cmplx(0.0_dp, 0.0_dp, kind=C_DOUBLE_COMPLEX)
