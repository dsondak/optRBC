...
Raloop: do ii = 1, nRa
            ...
            do alphai = 1,alphai_max
               ...
               if (alphai == alphai_max) then
                  ...
                  do while (ext == 1)
                     ...
                     ! Get solution with flow map
                     call flow_map(eta, nl_iter_data, temp)
                     imin = minloc(Nui)
                     Nui(imin) = eta
                     Li(imin) = Lx
                     if (eta < maxval(Nui)) then
                        ext = 0
                     end if
                  end do ! while ext = 1
               end if
            ...
            call fit_parab(Lmax,Numax, Li,Nui)

            alphamax = 2.0_dp * pi / Lmax

            write(8000, fmt=4000) Ra, Numax, alphamax, Lmax
            flush(8000)

            ! Now compute the optimal solution
            if (calc_opt) then
               xR =  pi / alphamax
               xL = -pi / alphamax
               Lx = Lmax
               ...
               ! Determine if writing data to vtk file
               call write_vtk_and_restart(nint(Ra), .false.) ! false = Fourier space
            end if
